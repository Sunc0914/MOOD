...
NSString *showMainViewNotification = @"showMainViewNotification";
...
+ (void)showImageOnView:(UIView *)container forTime:(NSTimeInterval)time
{
    ...
    UIButton *clearBtn = [[UIButton alloc]initWithFrame:CGRectMake(0, 0, SCREEN_WIDTH, SCREEN_HEIGHT-20)];
    clearBtn.backgroundColor = [UIColor clearColor];
    [clearBtn addTarget:self action:@selector(showAdDetail:) forControlEvents:UIControlEventTouchUpInside];
    ...
    dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(time * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        ...
        [UIView animateWithDuration:.25
                         ...
                         completion:^(BOOL finished) {
                             ...
                             [[NSNotificationCenter defaultCenter] postNotificationName:showMainViewNotification
                                                                                 object:nil];
...
- (void)connectionDidFinishLoading:(NSURLConnection *)connection
{
    ...
    //设置一个图片的存储路径
    NSString *imagePath=[NSString stringWithFormat:@"%@/Documents/%@.png",NSHomeDirectory(),@"mainAd"];
    
    //把图片直接保存到指定的路径（同时应该把图片的路径imagePath存起来，下次就可以直接用来取）
    [UIImagePNGRepresentation(iv.image) writeToFile:imagePath atomically:YES];
}
...
- (void)connection:(NSURLConnection *)connection didFailWithError:(NSError *)error
{
    ...
    if (UIImagePNGRepresentation(imgFromUrl) == nil) {
        
        data = UIImageJPEGRepresentation(imgFromUrl, 1);
        
    } else {
        
        data = UIImagePNGRepresentation(imgFromUrl);
    }
    
    monitor.imgData = [[NSMutableData alloc]initWithData:data];
